...
class CSVUploadTestCase(APITestCase):
    def setUp(self):
        ...
        self.inflow_subtype = TransactionSubType.objects.create(
            transaction_type=self.income_type, name="Inflow"
        )
        self.outflow_subtype = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Outflow"
        )
        self.buy_subtype = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Stock/ETF/Bond Purchase"
        )
        self.sell_subtype = TransactionSubType.objects.create(
            transaction_type=self.income_type, name="Investment Returns"
        )
        self.not_assigned_subtype = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Not assigned"
        )
    ...
    def test_csv_upload_requires_authentication(self):
        ...
        response = self.client.post(
            "/api/upload-csv/",
            {"file": csv_file, "bank_account": self.bank_account.id},
            format="multipart",
        )
        self.assertIn(
    ...
    def test_csv_upload_automatic_subtype_assignment_by_note(self):
        ...
        custom_subtype = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Custom Expense"
        )
    ...
    def test_csv_upload_automatic_subtype_assignment_by_isin(self):
        ...
        custom_stock_subtype = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Custom Stock Buy"
        )
        existing_transaction = Transaction.objects.create(
            user=self.user,
            amount=-100.00,
...
class PortfolioViewTestCase(APITestCase):
    ...
    def test_portfolio_with_single_buy_transaction(self, mock_fetch_prices):
        ...
        self.assertEqual(
            data["total_value"], 180.0
        )  # Should be current value with mocked price
    ...
    def test_portfolio_with_multiple_stocks(self, mock_fetch_prices):
        ...
        self.assertAlmostEqual(
            holdings_by_isin["US0378331005"]["value"], 190.0, places=2
        )
        self.assertAlmostEqual(
            holdings_by_isin["US5949181045"]["value"], 125.0, places=2
        )
        self.assertAlmostEqual(
            holdings_by_isin["US02079K3059"]["value"], 240.0, places=2
        )
    ...
    def test_portfolio_ignores_non_stock_transactions(self, mock_fetch_prices):
        ...
        regular_expense = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Regular Expense"
        )
        Transaction.objects.create(
            user=self.user,
            amount=-50.00,
...
class AdjustHoldingTestCase(APITestCase):
    ...
    def test_adjust_holding_buy_additional_shares(self):
        ...
        response = self.client.post(
            "/api/adjust-holding/",
            data=json.dumps(
                {"isin": "US0378331005", "new_shares": "10.0", "current_price": "25.0"}
            ),
        ...
        new_transaction = Transaction.objects.filter(
            user=self.user, isin="US0378331005"
        ).latest("created_at")
        self.assertEqual(new_transaction.transaction_subtype, self.buy_subtype)
        self.assertEqual(
            new_transaction.amount, Decimal("-125.00")
        )  # 5 * 25 * -1 = -125.00 not -250.00
    ...
    def test_adjust_holding_sell_shares(self):
        ...
        response = self.client.post(
            "/api/adjust-holding/",
            data=json.dumps(
                {"isin": "US0378331005", "new_shares": "5.0", "current_price": "25.0"}
            ),
            content_type="application/json",
        )
    ...
    def test_adjust_holding_create_new_position(self):
        ...
        # Check new transaction was created
        new_transaction = Transaction.objects.filter(
            user=self.user, isin="US5949181045"
        ).latest("created_at")
        self.assertEqual(new_transaction.transaction_subtype, self.buy_subtype)
    ...
    def test_adjust_holding_negative_shares_error(self):
        ...
        response = self.client.post(
            "/api/adjust-holding/",
            data=json.dumps(
                {"isin": "US0378331005", "new_shares": "-5.0", "current_price": "20.0"}
            ),
            content_type="application/json",
        )
    ...
    def test_adjust_holding_missing_isin_error(self):
        ...
        response = self.client.post(
            "/api/adjust-holding/",
            data=json.dumps({"new_shares": "10.0", "current_price": "20.0"}),
            content_type="application/json",
        )
    ...
    def test_adjust_holding_invalid_json(self):
        ...
        response = self.client.post(
            "/api/adjust-holding/", "invalid json", content_type="application/json"
        )
...
class PortfolioWithMockPriceTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # Create transaction subtypes for stocks
        self.buy_subtype = TransactionSubType.objects.create(
            transaction_type=self.expense_type, name="Buy"
        )
        self.sell_subtype = TransactionSubType.objects.create(
    ...
    def test_portfolio_price_fetch_failure_fallback(
        self, mock_get_history, mock_fetch_prices
    ):
